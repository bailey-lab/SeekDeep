...
int SeekDeepUtilsRunner::setupTarAmpAnalysis(
  ...
  setUp.setOption(pars.previousPopSeqsDir, "--previousPopSeqsDir",
	...
	auto files = njh::files::listAllFilesThrowOnDupSymlink(pars.inputDir.string(), false, {inputFilePat});
...
//				errorOutput << "\tSample: " << samp << std::endl;
//				for (const auto & sampFiles : njh::mapAt(rpOrganizer.readPairs_,samp)) {
//					errorOutput << "\t\t" << sampFiles << std::endl;
//				}
//			}
//		}
	...
	if(!pars.doNotGuessRecFlags){
		...
		investPars.pars.corePars_.pDetPars.primerWithin_ = analysisSetup.pars_.primerWithinToInvesitgate;
		...
		if(setUp.pars_.verbose_) {
			std::cout << "recommendations based on investigation: " << njh::conToStr(recFlags) << std::endl;
		}
		VecStr flagsToAdd;
		...
		for(const auto & recFlag : recFlags){
			...
			if(std::string::npos == currentExtraExtractorCmds.find(rflag)){
				...
				!(pars.techIsNanoporeOrPacbio() && "checkrevcomplementformids" == rflag) &&
				!(pars.techIsNanoporeOrPacbio() && njh::beginsWith(rflag, "midwithinstart") )) {
		...
		for (const auto & index : indexes) {
			...
			if (true) {
				...
				if(foundInputFile) {
					...
					for(const auto & sampleMid : njh::mapAt(analysisSetup.samples_, index).samples_ ){
					...
					//for (const auto & mid : analysisSetup.idsMids_->getMids()) {
						...
						for (const auto & tar : analysisSetup.indexToTars_[index]) {
							...
							if (1
									== njh::mapAt(analysisSetup.idsMids_->targets_,tar).overlapStatuses_.size()
									&& PairedReadProcessor::ReadPairOverLapStatus::NOOVERLAP
											== njh::mapAt(analysisSetup.idsMids_->targets_,tar).overlapStatuses_.front() &&
							...
							if (1 == njh::mapAt(analysisSetup.idsMids_->targets_,tar).overlapStatuses_.size() &&
									PairedReadProcessor::ReadPairOverLapStatus::NOOVERLAP == njh::mapAt(analysisSetup.idsMids_->targets_,tar).overlapStatuses_.front() &&
									analysisSetup.pars_.techIsIllumina()
											) {
...
//				}
  ...
  if(pars.techIsNanoporeOrPacbio()){
    processClusterTemplate += " --allowHomopolymerCollapse  ";
  ...
  exit
	...
	runAnalysisFile << "" << setUp.commands_.masterProgram_
			...
			<< std::endl;
  runAnalysisFile << "/bin/ls popClustering/*/analysis/selectedClustersInfo.tab.txt.gz | SeekDeep rBind --fill --files STDIN --delim tab --header --overWrite --out popClustering/allSelectedClustersInfo.tab.txt.gz" << std::endl;
	runAnalysisFile << "" << std::endl;
	//make file executable
