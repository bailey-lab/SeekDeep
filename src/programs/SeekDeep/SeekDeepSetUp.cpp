...
void SeekDeepSetUp::setUpExtractor(
  ...
  if (needsHelp()) {
    ...
    tempOut << "Extracts sequences from fasta/fastq or sff.txt files and some "
    ...
    std::cout << "examples: " << std::endl
              << "\tSeekDeep extractor -sff reads.sff.txt -id "
...
void SeekDeepSetUp::setUpClusterDown(
    ...
    bool& markChimeras, int& parFreqs, bool& bestMatch, int& bestMatchCheck,
...
void SeekDeepSetUp::setUpMultipleSampleCluster(
  ...
  if (needsHelp()) {
    ...
    tempOut << "SeekDeep multipleSampleClusters -fasta output.fasta "
               "-par pars.txt -population" << std::endl;
    tempOut << "SeekDeep multipleSampleClusters -fastq output.fastq -par "
  ...
  setOption(cutOff, "-cutoff", "FractionCutoff");
...
void SeekDeepSetUp::setUpMakeSampleDirectories(
  ...
  if (needsHelp()) {
    ...
    tempStream << "processClusters" << std::endl;
    ...
    std::cout << "examples, SeekDeep makesampleDirectories -file "
                 "names.tab.txt -dir clustering" << std::endl;
    exit(1);
  }
