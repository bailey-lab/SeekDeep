...
SampleFileNameGenerator::SampleFileNameGenerator(const bfs::path & idFnp,
		const bfs::path sampleFnp, bool noBarcodes) :
		idFnp_(idFnp), sampleFnp_(sampleFnp), noBarcodes_(noBarcodes) {
	...
	if(noBarcodes){
		...
		sampleTab.checkForColumnsThrow(VecStr{"library", "sample"}, __PRETTY_FUNCTION__);


		std::set<std::string> alreadyAddedLibraries;
		VecStr warnings;
		...
		for(const auto & row : sampleTab){
			...
			libraryFilesForSample_[sample].emplace_back(library);


			if(njh::in(library, alreadyAddedLibraries)){
				warnings.emplace_back(njh::pasteAsStr("Already have library ", library, " for sample ", sample));
			}
			alreadyAddedLibraries.emplace(library);
		}
		if(!warnings.empty()){
			std::stringstream ss;
...
void SampleFileNameGenerator::writeSampleNameFile(const OutOptions & sampleNamesOutOpts){
	if(noBarcodes_){
		VecStr headerForSampNamesFile{"#target", "sample"};
		uint32_t maxRunCount = 1;
		for(const auto & sample : libraryFilesForSample_){
		...
		for(const auto & sample : libraryFilesForSample_){
			sampleNamesOut << "all\t" << sample.first;
